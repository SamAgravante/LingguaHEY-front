...
export default function WordTranslation({ activityId }) {
  ...
  const handleChoiceEdit = (idx, text) => {
  ...
  const toggleEditCorrect = (text) => {
    ...
    const id = editingId;
    if (!editWord.trim()) return setMsgMap((m) => ({ ...m, [id]: "Word cannot be empty." }));
    if (editChoices.length < 3) return setMsgMap((m) => ({ ...m, [id]: "Add at least 3 choices." }));
    if (!editCorrect) return setMsgMap((m) => ({ ...m, [id]: "Select correct answer." }));

    setMsgMap((m) => ({ ...m, [id]: "Saving changes..." }));
    try {
      const form = new FormData();
      ...
      setMsgMap((m) => ({ ...m, [id]: "Updated successfully!" }));
      setEditingId(null);
      const res = await api.get(`/api/lingguahey/questions/activities/${activityId}`);
      setQuestions(res.data);
    } catch (error) {
      ...
      <Box sx={{ width: "100%", maxWidth: 900, mx: "auto" }}>
        ...
        {questions.map((q, idx) => (
          <Paper key={q.questionId} sx={{ bgcolor: "#232323", p: 4, borderRadius: 3, mb: 4 }} elevation={3}>
            <Typography variant="h6" fontWeight="bold" color="#81D4FA" sx={{ mb: 2 }}>
              {idx + 1}.
            </Typography>
              ...
              <Box sx={{ flex: 2 }}>
                ...
                <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1, mb: 2 }}>
                  {(editingId === q.questionId ? editChoices : q.choices).map((c, i) => {
                    ...
                    const isCorrect = editingId === q.questionId ? editCorrect.includes(text) : c.correct;
                    return (
                      <Chip
                        key={i}
                        label={
                        ...
                        onClick={() => editingId === q.questionId ? toggleEditCorrect(text) : null}
                        onDelete={() =>
                          editingId === q.questionId ? setEditChoices(editChoices.filter((_, j) => j !== i)) : null
                        }
                        sx={{
                ...
                <Box sx={{ display: "flex", justifyContent: "flex-end", gap: 2 }}>
                  {editingId === q.questionId ? (
                    <>
                      <Button variant="contained" onClick={handleSaveEdit} sx={{ bgcolor: "#4CAF50", fontWeight: "bold" }}>
                        Save
                      </Button>
                      <Button
                        variant="contained"
                        ...
                        sx={{ bgcolor: "#E57373", fontWeight: "bold" }}
                      >
                        Cancel
                      </Button>
                    </>
                    ...
                    <>
                      <Button
                        variant="contained"
                        onClick={() => startEdit(q)}
                        sx={{ bgcolor: "#81D4FA", color: "#000", fontWeight: "bold" }}
                      >
                        Edit
                      </Button>
                      <IconButton onClick={() => handleDelete(q.questionId)} sx={{ bgcolor: "#E57373", color: "#fff" }}>
                        <DeleteIcon />
                      </IconButton>
                    </>
                  )}
                ...
                {msgMap[q.questionId] && (
                  <Typography
                    color={msgMap[q.questionId].includes("failed") ? "#E57373" : "#81C784"}
        ...
        <Paper sx={{ bgcolor: "#232323", p: 4, borderRadius: 3 }} elevation={3}>
            ...
            <Box flex={2}>
              ...
              <Box sx={{ display: "flex", gap: 1, mb: 2 }}>
                ...
                <Button
                  ...
                  disabled={!inputChoice.trim() || newChoices.length >= 5}
                  sx={{ bgcolor: "#81D4FA", color: "#000" }}
                >
                  Add
                </Button>
              </Box>
              ...
              <Box sx={{ display: "flex", justifyContent: "flex-end", gap: 2 }}>
                ...
                <Button
                  ...
                  disabled={!newWord.trim() || newChoices.length < 3 || !correctAnswer}
                  sx={{ bgcolor: "#4CAF50" }}
                >
                  Save
                </Button>
          ...
          {newMessage && (
            <Typography color={newMessage.includes("failed") ? "#E57373" : "#81C784"} sx={{ mt: 3, textAlign: "center" }}>
              {newMessage}
            </Typography>
          )}
        </Paper>
